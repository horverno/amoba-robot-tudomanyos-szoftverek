...
namespace AmobaProject_Vision_Processor
{
    public class Camera
    {
        ...
        private GameboardDrawer prevBoard;
        ...
        Emgu.CV.UI.ImageBox ibOriginal;
        Emgu.CV.UI.ImageBox ibDetected;


        public Camera(Emgu.CV.UI.ImageBox ibOriginal, Emgu.CV.UI.ImageBox ibDetected)
        {
            try
            {
                calOk = false;
        ...
        private void ProcessFrame(object sender, EventArgs arg) {
            ...
            if (!calOk)
            {
                if (newGame)
                {
                    System.Console.WriteLine("Új pálya keresése.");
                    newGame = false;
                }
                ...
                if (boxlist.Count > 0)
                {
                    ...
                    if (calib.Calibrated() && x == y && (x == 4 || x == 3))
                    {
                        calOk = true;
                        System.Console.WriteLine("Pálya megtalálva.");
                    }
                }
            }
            else
            {
                ...
                DetectedImage = DrawGrid(DetectedImage, new Point(maxLeft, maxTop), new Point(maxRight, maxTop), new Point(maxLeft, maxBottom), new Point(maxRight, maxBottom), x, y); //az előző adatok alapján rárajzolja a képre a játékmezőt
                if (!GameboardDrawer.Compare(prevBoard, board))
                {
                    if (board.allFree())
                    {
                        System.Console.WriteLine("Új játék.");
                        calOk = false;
                        newGame = true;
                    }
                    else
                    {
                        System.Console.WriteLine("Bábu került fel vagy le.");
                    }
                }
            }
